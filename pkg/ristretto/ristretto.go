...
func mapToPoint(out *edwards25519.Point, t *field.Element) {
	...
	if _, err := out.SetExtendedCoordinates(&X, &Y, &Z, &T); err != nil {
		panic("ristretto: internal error: MAP generated invalid coordinates")
	}
}
...
func (e *Element) SetCanonicalBytes(in []byte) (*Element, error) {
	...
	if _, err := e.r.SetExtendedCoordinates(&X, &Y, &Z, &T); err != nil {
		panic("ristretto: internal error: DECODE generated invalid coordinates")
	}
	return e, nil
}
...
func (e *Element) BytesEd25519() []byte {
	// we can't just return the bytes of the underlying point, since it may not be of order 8.
	// so we do [8^{-1}][8]P to clear any cofactor
